package com.wong.dbutils;

import com.wong.domain.Product;
 ...
 * 参数sql：传递执行的select语句
 * 参数rsh：是接口ResultSetHandler的实现类对象
 * ResultSetHandler：表示的是查询后的结果集
 * 接口实现类：
 * BeanHandler：查询数据表的第一行数据，存储到JavaBean对象
 * BeanListHandler：查询数据表，数据的每一行数据存储到JavaBean对象，多个JavaBean对象，存储到List集合
 * ColumnLIstHandler：查询数据表中的一个列的数据，存储到List集合
 * ScalarHandler：适合单值查询
...
public class DBUtilsDemo02 {
    ...
    public static void beanHandler() throws SQLException {
        ...
        Product product = qr.query(sql, new BeanHandler<Product>(Product.class), 20);
        System.out.println(product);
    }
}
